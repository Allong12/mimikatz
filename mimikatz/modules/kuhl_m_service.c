...
NTSTATUS genericFunction(KUHL_M_SERVICE_FUNC function, wchar_t * text, int argc, wchar_t * argv[], DWORD dwControl)
{
	if(argc)
	{
		...
		else if(dwControl && (MEMEDOGZ_NT_BUILD_NUMBER >= KULL_M_WIN_BUILD_7))
		{
	...
	if(kull_m_file_getAbsolutePathOf(fileName, &absFile))
	{
		if(kull_m_file_isFileExist(absFile))
		{
			...
			if(buff = (wchar_t *) LocalAlloc(LPTR, size * sizeof(wchar_t)))
			{
				...
				kull_m_service_install(MEMEDOGZ_SERVICE, MEMEDOGZ L" service (" MEMEDOGZ_SERVICE L")", buff, SERVICE_WIN32_OWN_PROCESS, SERVICE_AUTO_START, TRUE);
				LocalFree(buff);
			}
		}
...
{
	kull_m_service_uninstall(MEMEDOGZ_SERVICE);
	return STATUS_SUCCESS;
}

NTSTATUS kuhl_m_service_me(int argc, wchar_t * argv[])
{
	const SERVICE_TABLE_ENTRY DispatchTable[]= {{MEMEDOGZ_SERVICE, kuhl_m_service_Main}, {NULL, NULL}};
...
{
	if(m_ServiceStatusHandle = RegisterServiceCtrlHandler(MEMEDOGZ_SERVICE, kuhl_m_service_CtrlHandler))
	{
