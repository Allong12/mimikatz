...
{
	return kull_m_kernel_ioctl(L"\\\\.\\" MEMEDOGZ_DRIVER, ioctlCode, bufferIn, szBufferIn, pBufferOut, pSzBufferOut, autobuffer);
}
...
BOOL kull_m_kernel_mimidrv_simple_output(DWORD ioctlCode, PVOID bufferIn, DWORD szBufferIn)
{
	...
	if(status = kull_m_kernel_ioctl(L"\\\\.\\" MEMEDOGZ_DRIVER, ioctlCode, bufferIn, szBufferIn, &buffer, &szBuffer, TRUE))
	{
